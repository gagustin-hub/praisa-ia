...
const UI_BUILD = 'v26-gateway-stable';
...
async function sendRecordedAudio(blob, mimeType) {
  ...
  try {
    ...
    if (falseTranscripts.some(text => normalized.includes(text))) {
      setVoiceUi(
        'idle',
        '⚠️ No detecté voz clara. Evita DroidCam Audio y elige el micrófono físico de la PC o de los audífonos.'
      );
      return;
    }
    ...
    setVoiceUi(
      'idle',
      '✅ Voz transcrita. Revisa el texto en la barra y pulsa enviar cuando esté correcto.'
    );
    chatInput()?.focus();
  } catch (error) {
