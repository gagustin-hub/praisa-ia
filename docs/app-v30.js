...
const UI_BUILD = 'v30-auto-conversation';
...
async function autoSendHandsFreeText(text) {
  ...
  if (!value) {
    helperText.textContent = 'No entendí esa respuesta. Te escucho otra vez…';
...
function startChat(username, password) {
  ...
  passInput.value = '';
  setTimeout(() => {
    helperText.textContent = 'Praisa IA ' + UI_BUILD + ' · Pulsa 🎙️ una vez: después solo conversa, sin tocar Enviar.';
    observeChatContext();
  }, 600);
}
