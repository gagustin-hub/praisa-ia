...
const UI_BUILD = 'v31-context-aware-voice';
...
function prepareHandsFreeReply(rawText) {
  ...
  if (isGarbageVoiceTranscript(value)) {
    return { ok:false, retry:true, reason:'Esa transcripción parece ruido o una frase automática.' };
  }
  ...
  if (expected === 'client') {
    if (value.length < 3) {
      return { ok:false,retry:true,reason:'No identifiqué el nombre o cuenta del cliente.' };
    }
  }
...
function startChat(username, password) {
  ...
  passInput.value = '';
  setTimeout(() => {
    helperText.textContent = 'Praisa IA ' + UI_BUILD + ' · Modo voz filtra respuestas según la pregunta actual.';
    observeChatContext();
  }, 600);
}
