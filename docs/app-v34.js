...
const UI_BUILD = 'v34-command-center';
...
function syncCommandCenterSummary() {
  ...
  if (status) {
    const tech = (document.getElementById('context-tech')?.textContent || '').trim();
    status.textContent = /pendiente|revisi[oó]n|requer/i.test(tech) ? 'Revisión' : 'Activa';
  }
}
